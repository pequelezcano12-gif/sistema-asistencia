...
class Alumno {
    ...
    public function create(array $data): int|false {
        $st = $this->db->prepare(
            "INSERT INTO alumnos (nombre, apellido, dni, fecha_nacimiento, email, telefono, direccion, cedula_padre, nombre_encargado, telefono_encargado, activo)
             VALUES (:n, :a, :d, :fn, :e, :t, :dir, :cp, :ne, :te, TRUE) RETURNING id"
        );
        $st->execute([
            ':n'=>$data['nombre'], ':a'=>$data['apellido'], ':d'=>$data['dni'],
            ':fn'=>$data['fecha_nacimiento'] ?: null,
            ':e'=>$data['email'] ?? null, ':t'=>$data['telefono'] ?? null,
            ':dir'=>$data['direccion'] ?? null,
            ':cp'=>$data['cedula_padre'] ?? null,
            ':ne'=>$data['nombre_encargado'] ?? null,
            ':te'=>$data['telefono_encargado'] ?? null,
        ]);
    ...
    public function update(int $id, array $data): bool {
        $st = $this->db->prepare(
            "UPDATE alumnos SET nombre=:n, apellido=:a, dni=:d, fecha_nacimiento=:fn,
             email=:e, telefono=:t, direccion=:dir, activo=:ac,
             cedula_padre=:cp, nombre_encargado=:ne, telefono_encargado=:te WHERE id=:id"
        );
        return $st->execute([
            ':n'=>$data['nombre'], ':a'=>$data['apellido'], ':d'=>$data['dni'],
            ':fn'=>$data['fecha_nacimiento'] ?: null,
