...
class Curso {
    ...
    public function getAll(): array {
        return $this->db->query(
            "SELECT c.*, COUNT(DISTINCT i.alumno_id) AS total_alumnos
             ...
             WHERE c.activo = TRUE
             GROUP BY c.id ORDER BY c.anio_lectivo DESC, c.nivel, c.nombre"
        )->fetchAll();
    }
    ...
    public function create(array $data): int|false {
        $st = $this->db->prepare(
            "INSERT INTO cursos (nombre, turno, anio_lectivo, nivel, especialidad)
             VALUES (:n, :t, :a, :ni, :es) RETURNING id"
        );
        $st->execute([':n'=>$data['nombre'],':t'=>$data['turno'],':a'=>$data['anio_lectivo'],
                      ':ni'=>$data['nivel'] ?? 'basica', ':es'=>$data['especialidad'] ?? 'ninguna']);
        return $st->fetchColumn();
    }

    public function update(int $id, array $data): bool {
        $st = $this->db->prepare(
            "UPDATE cursos SET nombre=:n, turno=:t, anio_lectivo=:a, activo=:ac, nivel=:ni, especialidad=:es WHERE id=:id"
        );
        return $st->execute([':n'=>$data['nombre'],':t'=>$data['turno'],':a'=>$data['anio_lectivo'],
                             ':ac'=>(bool)$data['activo'],':ni'=>$data['nivel'] ?? 'basica',
                             ':es'=>$data['especialidad'] ?? 'ninguna',':id'=>$id]);
    }
