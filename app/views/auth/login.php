        ...
        <div class="col-md-5 col-lg-4">
            ...
            <div class="card auth-card p-4">
                ...
                <form method="POST" autocomplete="off" novalidate>
                    ...
                    <div class="mb-3">
                        <label class="form-label fw-semibold small">Número de cédula / Usuario</label>
                        <div class="input-group">
                            <span class="input-group-text"><i class="bi bi-person-vcard text-muted"></i></span>
                            <input type="text" name="cedula" id="cedula" class="form-control"
                                   placeholder="Cédula o 'admin'"
                                   value="<?= htmlspecialchars($_SESSION['login_cedula'] ?? $_POST['cedula'] ?? '') ?>"
                                   maxlength="20" required autofocus
                                   autocomplete="off"
                                   oninput="checkMinPass()">
                            <?php unset($_SESSION['login_cedula']); ?>
                        </div>
                    </div>
...
function checkMinPass() {
    const cedula = document.getElementById('cedula').value.trim().toLowerCase();
    ...
    if (pass.length > 0 && pass.length < 8) {
        msg.innerHTML = '<span class="text-danger"><i class="bi bi-x-circle me-1"></i>Mínimo 8 caracteres</span>';
        btn.disabled = true;
    } else {
        msg.textContent = '';
        btn.disabled = false;
