...
class Usuario {
    ...
    public function createFromRegistro(array $data): int|false {
        $hash = password_hash($data['password'], PASSWORD_ARGON2ID);
        // Si es alumno, obtener nombre/apellido del registro de alumnos
        $alumno = $this->getAlumnoByCedula($data['cedula']);
        $nombre  = $alumno ? $alumno['nombre']   : ($data['nombre']   ?? '');
        $apellido= $alumno ? $alumno['apellido']  : ($data['apellido'] ?? '');

        // Si es profesor pre-registrado, actualizar su registro
