...
if (preg_match('#^/usuarios/(\d+)/editar$#', $path, $m))   { (new UsuarioController)->editar($m[1]);   return; }
if (preg_match('#^/usuarios/(\d+)/eliminar$#', $path, $m)) { (new UsuarioController)->eliminar($m[1]); return; }
